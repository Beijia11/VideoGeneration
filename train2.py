...
img_path='./outputs/_DEMO/ski/img/000001.jpg'
file_path='./outputs/_PKL/ski/smpl.pkl'
patch_size = 14
feat_dim= 384
dinov2_vits14 = torch.hub.load('facebookresearch/dinov2', 'dinov2_vits14')
glctx = dr.RasterizeGLContext(device='cuda:0')
...
img_tensor = transform(img)[:3].unsqueeze(0).to('cuda')
...
for frame in data:
    ...
    faces=faces.clone().to(torch.int32)
    rast_out, rast_out_db = dr.rasterize(glctx, homogeneous_vertices, faces, resolution=[newH, newW])
    rast_out=rast_out.to('cuda')
    ...
    # img = T.ToPILImage()(img_tensor.squeeze()).convert("RGB")
